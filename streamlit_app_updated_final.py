...
summary = (
    f"This audit-focused app forecasts Starbucks revenue using expected transactions ({future_txn}), CPI from FRED, and store count.\n"
    f"The extended model projects next quarter revenues between ${forecast_df_ext['revenue'].min():.2f}M and ${forecast_df_ext['revenue'].max():.2f}M.\n"
    f"Store count's coefficient is {store_coef:.2f}, with a p-value of {store_pval:.4f}, "
    f"indicating it {'is' if store_pval < 0.05 else 'is NOT'} statistically significant."
)

if risk_flag:
    summary += "\n\n**Risk Flag:** Significant variance detected in forecasts, suggesting possible revenue misstatement risk."
