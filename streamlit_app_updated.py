...
future_txn = st.slider("Select expected transactions for next quarter:", min_value=800, max_value=1200, value=1000, step=10)
...
future_index = data.index[-1] + pd.DateOffset(months=3)
...
# Add forecast to data for plotting
forecast_df = pd.DataFrame({'revenue': [forecasted_revenue]}, index=[future_index])
